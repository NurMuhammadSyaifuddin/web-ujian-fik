...
class Master_model extends CI_Model
{
    ...
    {
        $this->datatables->select('a.id_kelas, a.nama_kelas, b.id_jurusan, b.nama_jurusan');
        $this->datatables->from('kelas a');
    ...
    public function getAllMatkulById($id = null)
    {
        if ($id === null) {
            ...
            $this->db->from('dosen_matkul');
            $this->db->where('dosen_id', $id);
            $matkul = $this->db->get()->result();
            $id_matkul = [];
            foreach ($matkul as $j) {
                $id_matkul[] = $j->matkul_id;
            }
            if ($id_matkul === []) {
                $id_matkul = null;
            }
    ...
    public function getDosenByIdByCourse($id, $single = false)
    {
        if ($single === false) {
    ...
    {
        $this->datatables->select('dosen_matkul.id, dosen.id_dosen, dosen.nama_dosen, matkul.id_matkul, GROUP_CONCAT(matkul.nama_matkul) as nama_matkul');
        $this->datatables->from('dosen_matkul');
        $this->datatables->join('dosen', 'dosen_id=id_dosen');
        $this->datatables->join('matkul', 'matkul_id=id_matkul');
        $this->datatables->group_by('dosen.nama_dosen');
        return $this->datatables->generate();
